defmodule Mix.Tasks.Torch.Gen.Html do
  ...
  def run(args) do
    ...
    %{format: format} = parse_config!("torch.gen.html", args)

    ensure_phoenix_is_loaded("torch.gen.html")

    # First, backup the projects existing templates if they exist
    Enum.each(@commands, &backup_project_templates/1)

    # Inject the torch templates for the generator into the priv/
    # directory so they will be picked up by the Phoenix generator
    Enum.each(@commands, &inject_templates(&1, format: format))
    ...
    Enum.each(@commands, &remove_templates/1)

    # Restore the projects existing templates if present
    Enum.each(@commands, &restore_project_templates/1)
